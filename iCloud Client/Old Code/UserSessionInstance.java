...
public class UserSessionInstance {
	...
	public UserSessionInstance(String username, String password, boolean extended_login,
			boolean debugenabled, boolean announceConnections) {
	...
	public void connect() throws Exception {
		...
		String authString = "{\"apple_id\":" + "\"" + username + "\"" + ",\"password\":" + "\""
				+ password + "\"" + ",\"extended_login\":" + extended_login + "}";
		// Debug Output
		if (debugenabled) {
		...
		URL httpUrl = new URL("https://setup.icloud.com:443/setup/ws/1/login?"
				+ "clientBuildNumber=" + clientBnum + "&" + "clientId=" + UUID);
		...
		parseUserProps(conn.getResponseDataAsString());
		...
		userTokens = new UserTokens(conn.getResponseCookies());
	...
	public void disconnect() throws Exception {
		...
		Iterator<HttpCookie> iterator = userTokens.getTokens().iterator();
		...
		URL httpUrl = new URL(accountServer.get("url") + "/setup/ws/1/logout?"
				+ "clientBuildNumber=" + clientBnum + "&clientId=" + UUID + "&token=" + valCookie
				+ "&dsid=" + getUserID() + "&proxyDest=" + "p30-setup");
		conn.setServerUrl(httpUrl);
		conn.setRequestMethod("POST");
		conn.setRequestHeaders(headersMap);
		conn.setRequestCookies(userTokens.getTokens());
		conn.setPayload("{}");
		conn.connect();
	...
	public UserTokens getTokenManager() {
		return userTokens;
	}
	...
	private void abc123() throws Exception {
		URL httpUrl = new URL("https://p30-contactsws.icloud.com/co/startup?"
				+ "clientBuildNumber=" + clientBnum + "&" + "clientId=" + UUID + "&dsid="
				+ "8084583249" + "&locale=" + "en_US" + "&order=" + "last,first" + "&clientVersion"
				+ "2.1");
		...
		userTokens.updateTokens(conn.getResponseCookies());

		if (debugenabled) {
			for (HttpCookie cookie : userTokens.getTokens()) {
